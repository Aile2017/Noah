...
BOOL CArcViewDlg::onInit()
{
	...
	else
	{
		...
		for( int _i=0; _i<5; _i++ )
			::EnableWindow( item(items[_i]), FALSE );
	...
	case WM_GETMINMAXINFO:
		{
			...
			if( m_bShowToolbar )
			{
				...
				const int margin = 4, gap = 2, groupGap = 6, labelGap = 2;
...
void CArcViewDlg::rebuildContent()
{
	...
	else
	{
		static const UINT items[] = { IDC_REF,IDC_MELTEACH,IDC_SHOW,IDC_DDIR,IDC_DDIRLABEL };
		for( int i=0; i<5; i++ )
			::EnableWindow( item(items[i]), TRUE );
	}
                                ...
                                const char* sname, const char* lname )
{
	...
	//-- Recalculate extraction destination based on current settings
	{
		m_ddir = base;
		int mdf = mycnf().mkdir();
		if( mdf )
	...
	//-- Update display for destination folder
	{
		char longdir[MAX_PATH];
		if( ::GetLongPathName((const char*)m_ddir, longdir, MAX_PATH) == 0 )
		{
